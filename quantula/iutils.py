...
def resize_image(input):
    ...
    if qcd_type == 'image_data':
        ...
        for image in qcd_images:
            ...
            outfile = f"{output_file_dir}/images/{image_file_name}"
            #perform adjustments
            image_adjuster.resize_image(image=image_data, width=input['width'], height=input['height'], method=input['method'], outfile=outfile)

        #add source tracker
        #source_info = qcdutils.load_source_tracker(input['qcd_input'])
        #qcdutils.add_to_source_tracker(source_tracker_dict=source_info, input=input, operation='image_resizing')
